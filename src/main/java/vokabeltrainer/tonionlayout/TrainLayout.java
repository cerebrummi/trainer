   ...
   public String toString()
   {
      synchronized (self.getTreeLock())
      {
         ...
         else if (hmax != Integer.MAX_VALUE)
         {
            if (h <= hmin)
            {
               if (h < hmin && self.getParent() instanceof JViewport)
               {
                  System.out.println(
                        "OKAY: JViewport (usually part of JScrollPane) should show vertical scrollbar"
                              ...
                              + "\nis smaller than the MinimumHeight required by the components = "
                              + hmin);
               }
               else if (h < hmin)
               {
               ...
               System.out.println("component heights are OKAY:"
                     ...
                     + "\nthe MaximumHeight allowed by the components = "
                     + hmax);
            }
            else if (hmax < h)
            {
         ...
         for (int i = 0; i < ncomponents; i++)
         {
            if (wmax[i] < wmin[i])
            {
               System.err.println("ERROR in component " + i + " MinimumWidth = "
                     + wmin[i] + "\nis larger than" + "\nMaximumWidth = "
                     + wmax[i]);
               wmax[i] = wmin[i]; // error correction
            }
            // allocating available width w according to minimum widths vs.
            // wmintotal
            wfinal[i] = (int) ((wmin[i] / (float) wmintotal) * w);
            if (wmin[i] > wfinal[i])
            {
         ...
         for (int i = 0; i < ncomponents; i++)
         {
            System.out.println("Component " + i + " is set to width = "
                  + wfinal[i] + " and height = " + h);
            System.out
                  .println("Location is x = " + x + " and y = " + insets.top);
            x += wfinal[i] + hgap;
         }
      }
      return "\nTrainLayout toString() was called";
