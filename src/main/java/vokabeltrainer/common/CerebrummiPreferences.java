...
public interface CerebrummiPreferences {
    static void read() {
        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setDarkmodeOn(preferences.getBoolean(CerebrummiNodes.getDarkmodeNode(), false));
        } catch (Exception e) {
        }

        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setSoundOn(preferences.getBoolean(CerebrummiNodes.getSoundNode(), true));
        } catch (Exception e) {
        }

        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setChoosenExpressionPath(preferences.get(CerebrummiNodes.getChoosenExpressionPathNode(), null));
        } catch (Exception e) {
        }

        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setVolume(preferences.getFloat(CerebrummiNodes.getVolumeNode(), -20f));
        } catch (Exception e) {
        }

        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setLetterImagesOn(preferences.getBoolean(CerebrummiNodes.getLetterPicturesNode(), true));
        } catch (Exception e) {
        }
        ...
        try {
            ...
            List<String> possibleDatabases = Arrays.stream(Chapter.Database.values()).filter(database -> !database.equals(Database.IMPORTED)).filter(database -> !database.equals(Database.SELF)).filter(database -> !database.equals(Database.TO_BE_DETERMINED)).filter(database -> !database.equals(Database.UNKNOWN)).map(database -> database.name().toLowerCase()).collect(Collectors.toList());
        ...
        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setTranslationCode(TranslationCode.valueOf(preferences.get(CerebrummiNodes.getTranslationLanguage(), TranslationCode.de_original.name())));
        } catch (Exception e) {
        }
        ...
        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.initSchabbat_modus(preferences.getBoolean(CerebrummiNodes.getSchabbatModus(), false));
        } catch (Exception e) {
        }

        try {
            Preferences preferences = Preferences.userRoot().node(CerebrummiNodes.getNode());
            Settings.setLanguageInput(Selection.valueOf(preferences.get(CerebrummiNodes.getLanguageNode(), Selection.GERMAN.name())));
        } catch (Exception e) {
        }
    }
}
